...
class EntitlementControllerTest {
    ...
    private static final String CONSOLE = "https://console.vxture.com";
    ...
    void anUnreachablePlatformIsReportedAsUnavailableNotAsUnsubscribed() {
        EntitlementController controller = new EntitlementController(
                fixed(Entitlement.unavailable("ws-1", "tenderforge")), CONSOLE);
    ...
    void aWorkspaceThatNeverSubscribedIsAnAnswerNotAnOutage() {
        EntitlementController controller = new EntitlementController(
                fixed(Entitlement.none("ws-1", "tenderforge")), CONSOLE);
        ...
        assertThat(body.get("subscribeUrl"))
                .isEqualTo(CONSOLE + "/subscribe?product=tenderforge&intent=subscribe");
    }
    ...
    void refreshEvictsTheCallersOwnWorkspaceBeforeAsking() {
        ...
        new EntitlementController(recording(calls), CONSOLE).refresh(requestFor("ws-1"));
    ...
    void readingTheViewEvictsNothing() {
        ...
        new EntitlementController(recording(calls), CONSOLE).current(requestFor("ws-1"));
